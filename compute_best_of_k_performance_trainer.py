...
from transformers import AutoTokenizer, HfArgumentParser, pipeline, TrainingArguments, Trainer
import json
...
from util.DecSumDataset import get_score_from_output, process_func, loadDecSumdataset, \
    prepare_dataset_partition_for_best_of_k
from util.Metrics import compute_and_save_metrics
from TrainerPipeline import TrainerPipeline
...
class ScriptArguments:
    ...
    mode: Optional[str] = field(default="agent_only", metadata={
        "help": "Under multi-gpu mode, we can only support loading one model for multi-gpu inference,"
                "this means we can only compute agent_only or reward_only. However, we will save all outputs in the end so it is fine."})
...
if __name__ == '__main__':
    ...
    if script_args.mode == "reward_only":
        ...
        if "lora" not in reward_model_dir:
            ...
            try:
                ...
                logger.warning(
                    "Failed to load base model from peft config, trying to load base model from base model dir")
                reward_base_model = AutoModelForSequenceClassification.from_pretrained(script_args.base_model_dir,
                                                                                       # device_map="auto",
                                                                                       config=config,
                                                                                       # torch_dtype=torch.float16,
                                                                                       )
            reward_model = PeftModel.from_pretrained(reward_base_model, reward_model_dir)
            # except Exception as e:
            #     logger.warning(e)
            #     # print stack trace
            #     traceback.print_exc()
            #     reward_model = peft.from_
            try:
                logger.warning("Trying to load tokenizer from reward model dir")
                tokenizer = AutoTokenizer.from_pretrained(reward_model_dir)
            except:
                logger.warning(
                    "Failed to load tokenizer from reward model dir, trying to load tokenizer from base model dir")
        ...
        reward_trainer = Trainer(model=reward_model, args=training_args, tokenizer=tokenizer,
                                 data_collator=default_data_collator)
        reward_pipe = TrainerPipeline(reward_trainer, tokenizer)
    agent_pipe = pipeline(task="text-classification", model=agent_model_dir, device=0)
    ...
    func_get_score_from_output = lambda predict, label: get_score_from_output(predict, label,
                                                                              compute_acc=script_args.criterion == "acc")
    for split, file_path in zip(['test'], [data_args.test_file]):
        ...
        summary_data, summary_pointers, _raw_dataset, dataset, original_test_ds = prepare_dataset_partition_for_best_of_k(
            file_path=file_path,
            split=split,
            output_eval_dir=output_eval_dir,
            processed_summary_dump_path=processed_summary_dump_path,
        ...
        try:
            ...
            if len(reward_outputs) == 0 and script_args.mode == "reward_only":
                raise Exception("No reward outputs found in cache")
        except Exception as e:
            logger.warning(e)
            logger.warning("Failed to load cached sentiment outputs, running sentiment analysis")
            if script_args.mode == "agent_only":
                # sent_outputs = []
                # sent_original_outputs = []
                time0 = time.time()
                logger.warning("Start generating fact-verification outputs (via agent pipeline)")
                sent_outputs = list(agent_pipe(KeyPairDataset(dataset, "claim", "evidence"), **sent_kwargs))
                logger.warning("Finished generating fact-verification outputs (via agent pipeline), took {}s".format(
                    time.time() - time0))
                logger.warning(
                    "Start generating fact-verification outputs (on original_test_ds -- w/o adding summary so no expert simulation, "
                    "via agent pipeline)")
                time0 = time.time()
                sent_original_outputs = list(
                    agent_pipe(KeyPairDataset(original_test_ds, "claim", "evidence"), **sent_kwargs))
                logger.warning(
                    "Finished generating fact-verification outputs (on original_test_ds, via agent pipeline), took {}s".format(
                        time.time() - time0))
                reward_outputs = []
            # if script_args.mode == "reward-only":
            else:
                assert os.path.exists(cache_path), "cache path {} does not exist".format(cache_path)
                cache_data = torch.load(cache_path)
                assert SHAkey in cache_data, "bug: SHAkey does not exist in cache"
                sent_outputs, sent_original_outputs, reward_outputs = cache_data[SHAkey]
                assert len(sent_outputs) > len(reward_outputs) and len(sent_original_outputs) > len(
                    reward_outputs), "bug: len(sent_outputs) > len(reward_outputs) and len(sent_original_outputs) > len(reward_outputs)"
                logger.warning(f"Start generating pragmatic-selection outputs (via reward pipeline), currently: {len(sent_outputs)} - {len(reward_outputs)} = {len(sent_outputs) - len(reward_outputs)}")
                processed_dataset = dataset.map(
                    lambda example: reward_pipe.preprocess_function(example, "claim", "evidence",
                                                                    data_args.max_seq_length), batched=True,
                    desc="Running tokenizer on dataset")
                reward_outputs = reward_pipe(processed_dataset)
            # time0 = time.time()
            # logger.warning("Start generating pragmatic-selection outputs (via reward pipeline)")
            # We now do it using two-stage way -- at the first stage, generate all sentiment_pipe outputs, and then
            # at the second stage, generate all reward_pipe outputs
            # if "llama" in reward_model_dir:
            #     reward_outputs = []
            ...
            if os.path.exists(cache_path):
                ...
                if not script_args.reset_cache:
                    assert SHAkey in cache_data, "bug: SHAkey should already exists in cache, and you do not want to reset cache"
            else:
                cache_data = {}
        ...
        try:
            logger.warning("Start computing and saving metrics")
            if script_args.mode == "reward_only":
                if reward_pipe.trainer.is_world_process_zero():
                    compute_and_save_metrics(reward_outputs, sent_outputs, sent_original_outputs, script_args,
                                             summary_data,
        ...
        # if len(reward_outputs) == len(sent_outputs) and len(sent_outputs) == len(sent_original_outputs):
        #     normal_scores = dict()
        ...
        #         summary_max_scores[category].append(
        #             summary_data[i]["performance"][np.argmax(summary_data[i]["reward"])])
        ...
        #         summary_min_scores[category].append(
        #             summary_data[i]["performance"][np.argmin(summary_data[i]["reward"])])
        ...
        #                  f"eval_{split}_summary_min.pkl", f"eval_{split}_fitness_mean.pkl",
        #                  f"eval_{split}_fitness_max.pkl",
        #                  f"eval_{split}_fitness_min.pkl", f"eval_{split}_ground_truth_max.pkl",
        #                  f"eval_{split}_ground_truth_min.pkl"],
        #                 [normal_scores[category], summary_mean_scores[category], summary_max_scores[category],
        #                  summary_min_scores[category], fitness_mean_scores[category],
        #                  fitness_max_scores[category], fitness_min_scores[category], ground_truth_max_scores[category],
        #                  ground_truth_min_scores[category]]):
        ...
        #                 all_mses[
        #                     output_filename.replace(".pkl", "").replace(f"eval_{script_args.criterion}_{split}_", "")] = \
        #                 res['avg']
        ...
        #             os.path.join(output_summary_dir,
        #                          f"all_{script_args.criterion}s_{split}_{category}{script_args.exp_name}.pkl"), "wb"))
        #         print(all_mses)
        #
        #     torch.save(summary_data, os.path.join(output_summary_dir,
        #                                           f"summary_data_{split}{script_args.exp_name}_{script_args.criterion}.pt"))
