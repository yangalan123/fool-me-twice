...
export DEFAULT_TORCH_EXTENSION_PATH=/home/chenghao/env_py38/lib/python3.8/site-packages/deepspeed/ops/op_builder/builder.py
...
#--test_file ${root_dir}/${domain}/test_w_summary.json \
      ...
      #--output_dir ./agent_training_output_epoch3_filter_chain_rest_train${suffix}/city/${domain}
          ...
          #--label_column rank \
    output_dir=${output_root_dir}/${domain}/agent_difference_dataset/validation_set_dbt_agent_training_output_epoch${epoch}${suffix}_w_noise_training
    if [[ ! -f ${output_dir}/pytorch_model.bin ]]; then
        deepspeed --master_port 6100${SLURM_ARRAY_TASK_ID} agent_training_slurm.py \
          ...
          --train_file ${root_dir}/${domain}/agent_difference_dataset_w_noise_sample16/dev_noise_difference.json \
          --validation_file ${root_dir}/${domain}/agent_difference_dataset_w_noise_sample16/dev_noise_difference.json \
          --test_file ${root_dir}/${domain}/agent_difference_dataset_w_noise_sample16/test_noise_difference.json \
          --max_seq_length 2048 \
